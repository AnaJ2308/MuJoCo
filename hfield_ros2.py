#!/usr/bin/env python3
"""
Unified ROS2 -> MuJoCo heightfield viewer (single process, no TCP)

- Subscribes to THREE PointCloud2 topics
- TFs each cloud into a common Z-up frame (default: base_link)
- Crops to a square ROI and (optionally) voxel-downsamples
- Aggregates per-cell Z (mean/max/median) into an (nrow x ncol) grid in METERS
- Enforces a ground plane for empty cells (so the base is always flat)
- Maps meters -> [0,1] for MuJoCo and updates the heightfield live
"""

import time
import numpy as np
from typing import Dict, Optional

# --- ROS2 ---
import rclpy
from rclpy.node import Node
from rclpy.duration import Duration
from rclpy.qos import QoSProfile, ReliabilityPolicy, HistoryPolicy
from rclpy.executors import SingleThreadedExecutor
from sensor_msgs.msg import PointCloud2
from sensor_msgs_py import point_cloud2
from tf2_ros import Buffer, TransformListener
from tf2_sensor_msgs.tf2_sensor_msgs import do_transform_cloud

# --- MuJoCo ---
import mujoco
from mujoco import viewer


# ===================== Utilities =====================

def pc2_to_xyz_np(msg: PointCloud2) -> np.ndarray:
    it = point_cloud2.read_points(msg, field_names=('x','y','z'), skip_nans=True)
    arr = np.fromiter(it, dtype=[('x','f4'),('y','f4'),('z','f4')])
    if arr.size == 0:
        return np.empty((0,3), np.float32)
    return np.column_stack((arr['x'], arr['y'], arr['z'])).astype(np.float32, copy=False)

def voxel_downsample(xyz: np.ndarray, voxel: float) -> np.ndarray:
    if xyz.shape[0] == 0 or voxel <= 0:
        return xyz
    q = np.floor(xyz / voxel).astype(np.int64)
    _, keep = np.unique(q, axis=0, return_index=True)
    return xyz[np.sort(keep)]

def rasterize_agg(points_xyz: np.ndarray,
                  nrow: int, ncol: int,
                  bounds: tuple[float,float,float,float],
                  agg: str = 'mean',
                  prev_grid: np.ndarray | None = None,
                  smooth_passes: int = 1) -> np.ndarray:
    xmin, xmax, ymin, ymax = bounds
    if points_xyz.size == 0:
        return prev_grid if prev_grid is not None else np.zeros((nrow, ncol), np.float32)

    x = points_xyz[:, 0]; y = points_xyz[:, 1]; z = points_xyz[:, 2].astype(np.float32, copy=False)

    ix = ((x - xmin) / (xmax - xmin) * (ncol - 1)).astype(np.int32)
    iy = ((y - ymin) / (ymax - ymin) * (nrow - 1)).astype(np.int32)
    np.clip(ix, 0, ncol - 1, out=ix)
    np.clip(iy, 0, nrow - 1, out=iy)
    flat_idx = iy * ncol + ix

    if agg == 'mean':
        counts = np.bincount(flat_idx, minlength=nrow * ncol)
        sums   = np.bincount(flat_idx, weights=z, minlength=nrow * ncol)
        Hflat = np.divide(
            sums, counts,
            out=np.full(nrow * ncol, np.nan, np.float32),
            where=counts > 0
        )
        H = Hflat.reshape(nrow, ncol)
    elif agg == 'median':
        from collections import defaultdict
        buckets = defaultdict(list)
        for idx, zz in zip(flat_idx, z):
            buckets[idx].append(zz)
        H = np.full((nrow, ncol), np.nan, np.float32)
        for idx, vals in buckets.items():
            i = idx // ncol; j = idx % ncol
            H[i, j] = float(np.median(vals))
    else:  # 'max'
        Hflat = np.full(nrow * ncol, -np.inf, np.float32)
        np.maximum.at(Hflat, flat_idx, z)
        H = Hflat.reshape(nrow, ncol)
        H[H == -np.inf] = np.nan

    # Fill empty cells from previous grid to reduce flicker
    if prev_grid is not None:
        mask = np.isnan(H)
        if mask.any():
            H[mask] = prev_grid[mask]

    # Light smoothing over NaNs/speckle
    for _ in range(max(smooth_passes, 0)):
        if not np.isnan(H).any():
            break
        Hp = np.pad(H, 1, mode='edge')
        neigh = np.stack([
            Hp[0:-2,1:-1], Hp[2:,1:-1], Hp[1:-1,0:-2], Hp[1:-1,2:],
            Hp[0:-2,0:-2], Hp[0:-2,2:], Hp[2:,0:-2], Hp[2:,2:]
        ], axis=0)
        with np.errstate(invalid='ignore'):
            m = np.nanmean(neigh, axis=0)
        nanmask = np.isnan(H) & ~np.isnan(m)
        H[nanmask] = m[nanmask]

    if np.isnan(H).any():
        H[np.isnan(H)] = np.nanmin(H)

    return H.astype(np.float32, copy=False)


# ===================== ROS2 Node =====================

class MultiCloudNode(Node):
    def __init__(self):
        super().__init__('hfield_unified_viewer')

        # ---- Parameters (override via ROS args if you want) ----
        self.declare_parameter('target_frame', 'base_link')
        self.declare_parameter('topics', [
            '/topic_1/cam_1/depth/color/points',
            '/topic_2/cam_2/depth/color/points',
            '/topic_3/cam_3/depth/color/points',
        ])
        self.declare_parameter('half_extent_m', 6.0)   # ROI is [-L,+L]^2 in (x,y)
        self.declare_parameter('nrow', 160)
        self.declare_parameter('ncol', 160)
        self.declare_parameter('voxel_size_m', 0.08)
        self.declare_parameter('clip_percentiles', []) # e.g., [2.0, 98.0]
        self.declare_parameter('flip_x', False)
        self.declare_parameter('flip_y', True)
        self.declare_parameter('aggregate', 'mean')    # 'mean'|'max'|'median'
        self.declare_parameter('smooth_passes', 1)
        self.declare_parameter('zmin_fixed', 0.20)     # meters (ground plane)
        self.declare_parameter('zmax_fixed', 5.0)      # unused here, but kept

        # ---- Viewer mapping (meters -> [0,1]) ----
        # FLOOR: everything <= FLOOR becomes the flat base (0.0)
        # HEAD:  1.0 in hfield == HEAD meters above FLOOR. Make big to avoid clipping.
        self.declare_parameter('viewer_floor_m', 0.20)
        self.declare_parameter('viewer_headroom_m', 2.0)
        self.declare_parameter('viewer_base_plate_m', 0.0001)

        # Pull parameter values
        self.target_frame = self.get_parameter('target_frame').get_parameter_value().string_value
        self.topics       = [str(s) for s in self.get_parameter('topics').get_parameter_value().string_array_value]
        self.L            = float(self.get_parameter('half_extent_m').value)
        self.nrow         = int(self.get_parameter('nrow').value)
        self.ncol         = int(self.get_parameter('ncol').value)
        self.voxel        = float(self.get_parameter('voxel_size_m').value)
        self.clip_pcts    = list(self.get_parameter('clip_percentiles').value) or []
        self.flip_x       = bool(self.get_parameter('flip_x').value)
        self.flip_y       = bool(self.get_parameter('flip_y').value)
        self.aggregate    = str(self.get_parameter('aggregate').value).lower()
        self.smooth_passes= int(self.get_parameter('smooth_passes').value)
        zmin_param        = self.get_parameter('zmin_fixed').value
        self.zmin_fixed   = float(zmin_param) if zmin_param is not None else None

        self.viewer_floor = float(self.get_parameter('viewer_floor_m').value)
        self.viewer_head  = float(self.get_parameter('viewer_headroom_m').value)
        self.viewer_base  = float(self.get_parameter('viewer_base_plate_m').value)

        self.cell_size = (2.0*self.L)/max(self.nrow,self.ncol)
        if self.voxel <= 0:
            self.voxel = self.cell_size

        # TF + subscriptions
        self.tf_buffer = Buffer(cache_time=Duration(seconds=10.0))
        self.tf_listener = TransformListener(self.tf_buffer, self)
        qos = QoSProfile(reliability=ReliabilityPolicy.BEST_EFFORT,
                         history=HistoryPolicy.KEEP_LAST, depth=3)
        self._latest: Dict[str, Optional[PointCloud2]] = {t: None for t in self.topics}
        self._seen_first = {t: False for t in self.topics}
        for t in self.topics:
            self.create_subscription(PointCloud2, t, self._make_cb(t), qos)

        self._grid: Optional[np.ndarray] = np.zeros((self.nrow,self.ncol), np.float32)
        self._pts_last = 0
        self._seq = 0
        self.create_timer(1.0, self._stats)

        self.get_logger().info(
            f'HField unified viewer up: ROI {2*self.L:.1f}m, grid {self.nrow}x{self.ncol}, voxel {self.voxel*100:.1f}cm')

    def _make_cb(self, topic):
        def _cb(msg: PointCloud2):
            if not self._seen_first[topic]:
                self.get_logger().info(f'First message from {topic} (frame={msg.header.frame_id})')
                self._seen_first[topic] = True
            self._latest[topic] = msg
        return _cb

    def _cloud_to_xyz_in_target(self, msg: PointCloud2) -> np.ndarray:
        if msg.header.frame_id and msg.header.frame_id != self.target_frame:
            try:
                tf = self.tf_buffer.lookup_transform(self.target_frame, msg.header.frame_id, rclpy.time.Time())
                msg = do_transform_cloud(msg, tf)
            except Exception as e:
                self.get_logger().warn(f'TF {msg.header.frame_id}->{self.target_frame} failed: {e}')
                return np.empty((0,3), np.float32)
        return pc2_to_xyz_np(msg)

    def get_concat_latest_xyz(self) -> np.ndarray:
        clouds = []
        for t in self.topics:
            m = self._latest.get(t)
            if m is None: continue
            xyz = self._cloud_to_xyz_in_target(m)
            if xyz.size: clouds.append(xyz)
        if not clouds:
            return np.empty((0,3), np.float32)
        xyz = np.concatenate(clouds, axis=0)
        L = self.L
        sel = (xyz[:,0]>=-L)&(xyz[:,0]<=L)&(xyz[:,1]>=-L)&(xyz[:,1]<=L)
        xyz = xyz[sel]
        if xyz.size==0: return xyz
        if len(self.clip_pcts)==2:
            lo,hi = np.percentile(xyz[:,2], self.clip_pcts)
            xyz[:,2] = np.clip(xyz[:,2], lo, hi)
        return voxel_downsample(xyz, self.voxel)

    def rasterize(self) -> np.ndarray:
        xyz = self.get_concat_latest_xyz()
        self._pts_last = int(xyz.shape[0])
        bounds = (-self.L, self.L, -self.L, self.L)

        # Keep a handle to previous grid BEFORE overwriting (for flicker fill)
        prev = None if (self.zmin_fixed is not None) else self._grid

        # Bin points -> per-cell Z (meters)
        H = rasterize_agg(
            xyz, self.nrow, self.ncol, bounds,
            agg=self.aggregate,
            prev_grid=prev,
            smooth_passes=self.smooth_passes
        )

        # Enforce ground plane
        if self.zmin_fixed is not None:
            ground = float(self.zmin_fixed)
            nanmask = ~np.isfinite(H)
            if nanmask.any():
                H[nanmask] = ground
            np.maximum(H, ground, out=H)
        else:
            if prev is not None:
                mask = ~np.isfinite(H)
                if mask.any():
                    H[mask] = prev[mask]
            if np.isnan(H).any():
                H[np.isnan(H)] = np.nanmin(H)

        # Optional flips for alignment
        if self.flip_y: H = np.flipud(H)
        if self.flip_x: H = np.fliplr(H)

        # Force outer rim to a fixed floor height so there's always a base border
        EDGE_FLOOR = float(self.viewer_floor)
        H[0,  :] = EDGE_FLOOR
        H[-1, :] = EDGE_FLOOR
        H[:,  0] = EDGE_FLOOR
        H[:, -1] = EDGE_FLOOR

        self._grid = H.astype(np.float32, copy=False)
        self._seq += 1
        return self._grid

    def to_h01_meters(self, Hm: np.ndarray) -> np.ndarray:
        """Map meters -> [0,1] for MuJoCo heightfield."""
        # 0.0 corresponds to FLOOR; 1.0 corresponds to FLOOR + HEAD
        return np.clip((Hm - self.viewer_floor) / self.viewer_head, 0.0, 1.0).astype(np.float32, copy=False)

    def _stats(self):
        self.get_logger().info(f'pts={self._pts_last}  grid={self.nrow}x{self.ncol}  seq={self._seq}')


# ===================== Main (Viewer + ROS in one loop) =====================

def build_model(nrow: int, ncol: int, L: float, head: float, base_plate: float):
    xml = f'''
<mujoco model="hf_viewer">
  <option timestep="0.01" gravity="0 0 -9.81"/>
  <asset>
    <!-- size: (x-extent, y-extent, vertical scale for 1.0, base thickness) -->
    <hfield name="terrain" nrow="{nrow}" ncol="{ncol}" size="{L} {L} {head} {base_plate}"/>
  </asset>
  <worldbody>
    <geom type="hfield" hfield="terrain" pos="0 0 0" rgba="0.7 0.85 0.7 1"/>
    <body pos="0 0 1.0">
      <freejoint/>
      <geom type="sphere" size="0.08" density="2000" rgba="0.9 0.2 0.2 1"/>
    </body>
  </worldbody>
</mujoco>'''
    model = mujoco.MjModel.from_xml_string(xml)
    data  = mujoco.MjData(model)
    hid   = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_HFIELD, 'terrain')
    adr   = model.hfield_adr[hid]
    return model, data, hid, adr

def main():
    rclpy.init()
    node = MultiCloudNode()

    # Build MuJoCo model once using node params
    model, data, hid, adr = build_model(
        nrow=node.nrow, ncol=node.ncol, L=node.L,
        head=node.viewer_head, base_plate=node.viewer_base
    )
    print(f"[viewer] floor={node.viewer_floor:.3f} m, headroom={node.viewer_head:.3f} m, base={node.viewer_base:.5f} m")

    # Create a ROS executor but DON'T block; we'll pump it in the render loop
    exe = SingleThreadedExecutor()
    exe.add_node(node)

    # Prime first frame
    Hm = node.rasterize()
    H01 = node.to_h01_meters(Hm)
    model.hfield_data[adr: adr + node.nrow*node.ncol] = H01.ravel(order='C')

    with viewer.launch_passive(model, data) as v:
        # Camera setup
        v.cam.lookat[:] = (0.0, 0.0, 0.3)
        v.cam.azimuth   = 90
        v.cam.elevation = -20
        v.cam.distance  = 7.0

        # Ensure first upload
        if hasattr(v, 'update_hfield'):
            v.update_hfield(hid)
        else:
            try: mujoco.mjr_uploadHField(model, hid, v.context)
            except Exception: pass

        # Main loop: spin ROS once, rasterize, update hfield, step physics, render
        target_dt = 1.0/30.0  # ~30 Hz viewer update; adjust as you like
        last = time.time()
        while v.is_running():
            start = time.time()
            # Process ROS callbacks (non-blocking)
            exe.spin_once(timeout_sec=0.0)

            # Build latest grid in meters
            Hm = node.rasterize()
            H01 = node.to_h01_meters(Hm)

            # Upload
            model.hfield_data[adr: adr + node.nrow*node.ncol] = H01.ravel(order='C')
            if hasattr(v, 'update_hfield'):
                v.update_hfield(hid)
            else:
                try: mujoco.mjr_uploadHField(model, hid, v.context)
                except Exception: pass

            # Step & render
            mujoco.mj_step(model, data)
            v.sync()

            # Simple pacing
            dt = time.time() - start
            if dt < target_dt:
                time.sleep(target_dt - dt)

    # Cleanup
    exe.remove_node(node)
    node.destroy_node()
    rclpy.shutdown()


if __name__ == '__main__':
    main()


